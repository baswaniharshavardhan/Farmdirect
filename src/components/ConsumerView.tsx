...
import { HeroEcosystem3D } from './HeroEcosystem3D';
...
export const ConsumerView: React.FC = () => {
  ...
  const handleCheckoutSubmit = (e: React.FormEvent) => {
    ...
    // Live tracking is ONLY activated for bulk commercial orders, NOT sample normal buyers
    if (currentUser.buyerType === 'bulk') {
  ...
  // Active tracking order is ONLY resolved for bulk buyers, removed for normal buyers
  const activeTrackingOrder = useMemo(() => {
    if (!activeTrackingOrderId || currentUser.buyerType !== 'bulk') return null;
    return (
      orders.find((o) => o.id === activeTrackingOrderId) ||
      (checkoutSuccessOrder?.id === activeTrackingOrderId ? checkoutSuccessOrder : null)
    );
  }, [activeTrackingOrderId, orders, checkoutSuccessOrder, currentUser]);
  ...
  return (
    <div className="space-y-6" id="consumer-buyer-app">
      {/* 3D Web Reimagined Hero Ecosystem Presentation */}
      <HeroEcosystem3D
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            {currentUser.buyerType === 'bulk' ? (
              ...
              >
                ...
                <span>Track Bulk Route Fleet</span>
              </button>
            ) : (
              <div className="flex items-center gap-2 text-xs font-bold text-emerald-200 bg-emerald-950/70 px-3.5 py-2 rounded-xl border border-emerald-500/40">
                <Clock className="w-4 h-4 text-emerald-400" />
                <span>Direct Eco-Van Delivery Confirmed</span>
              </div>
            )}
          </div>
        </div>
            ...
            <div>
              ...
              <div className="mt-2.5 flex items-center gap-3 flex-wrap">
                {currentUser.buyerType === 'bulk' ? (
                  ...
                  <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-800 bg-emerald-100/90 px-3 py-1 rounded-lg">
                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-700" /> Direct Delivery Dispatched
                  </span>
                )}
                <button
                  type="button"
      ...
      {/* Section Heading with Attractive Editorial Typography */}
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-2 pt-1 pb-1">
        <div>
          <div className="flex items-center gap-2">
            <span className="font-mono text-[10px] font-bold text-emerald-700 uppercase tracking-widest bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
              Verified Daily Harvest
            </span>
            <span className="font-script text-xl font-bold text-amber-700">Direct From Orchards</span>
          </div>
          <h2 className="attractive-headline text-2xl sm:text-3xl font-bold text-neutral-900 mt-0.5">
            Seasonal Regional Produce
          </h2>
        </div>
        <p className="text-xs text-neutral-500 font-sans">
          Showing <span className="font-mono font-bold text-emerald-800">{filteredProducts.length}</span> verified farm-direct lots
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredProducts.map((product) => {
          ...
          return (
            <div
              key={product.id}
              className="card-3d bg-white border border-emerald-950/10 hover:border-emerald-500/40 rounded-2xl overflow-hidden shadow-[0_10px_25px_-5px_rgba(0,0,0,0.06)] hover:shadow-[0_22px_40px_-10px_rgba(6,78,59,0.18)] transition-all duration-300 flex flex-col group"
              ...
              <div className="p-4 flex-1 flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between text-xs text-neutral-500 mb-1">
                    <span className="font-serif-harvest font-bold text-emerald-800 truncate">{product.farmName}</span>
                    <span className="flex items-center gap-1 flex-shrink-0 text-neutral-400 font-mono text-[10px]">
                      <MapPin className="w-3 h-3 text-emerald-600" /> {product.location ? product.location.split(',')[0] : 'Regional Farm'}
                    </span>
                  </div>

                  <h3 className="theme-heading font-serif-harvest font-bold text-neutral-900 text-base sm:text-lg leading-tight mb-1">
                    {product.name}
                  </h3>

                  <p className="text-neutral-500 text-xs line-clamp-2 leading-relaxed mb-2 font-sans">
                    {product.description}
                  </p>

                  {/* Handwritten Freshness Guarantee Seal */}
                  <div className="flex items-center justify-between py-1 px-2 rounded-lg bg-emerald-50/70 border border-emerald-100 mb-3">
                    <span className="font-script text-sm font-bold text-emerald-800">
                      🌱 Direct harvest from soil
                    </span>
                ...
                <div className="space-y-3 pt-2 border-t border-neutral-150">
                  ...
                  <div className="flex items-baseline justify-between">
                    <div>
                      ...
                      <span className="text-xs text-neutral-500 font-medium font-sans ml-1">/ {product.unit}</span>
                    </div>

                    <button
                      type="button"
                      onClick={() => setInspectingProduct(product)}
                      className="text-[11px] text-emerald-700 hover:text-emerald-900 font-bold underline font-mono"
                    >
                      Audit ₹ Breakdown
                    </button>
                  </div>
