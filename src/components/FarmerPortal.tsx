...
import { FarmerDailyOverview } from './FarmerDailyOverview';
import { TransportAvailabilitySection } from './TransportAvailabilitySection';
...
export const FarmerPortal: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'daily' | 'transport' | 'inventory' | 'orders' | 'payouts'>('daily');
  ...
  return (
    <div className="space-y-6" id="farmer-portal-app">
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl p-5 shadow-xs space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="theme-heading font-serif-harvest text-xl sm:text-2xl font-black text-neutral-900 tracking-tight">
                  {activeFarm.name}
                </h2>
                ...
                <span className="px-2 py-0.5 text-[11px] font-bold rounded-md bg-blue-50 text-blue-800 border border-blue-200 flex items-center gap-1 font-mono text-[10px]">
              ...
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                ...
                <span className="font-script text-sm font-bold text-emerald-700">
                  ~ 100% Regenerative Harvested
                </span>
              </div>
              <p className="text-xs text-neutral-600 mt-1 max-w-xl line-clamp-1 font-sans">{activeFarm.bio}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3 border-t border-neutral-150">
          <div className="p-3 bg-emerald-50/70 border border-emerald-200 rounded-xl">
            ...
            <div className="text-xl sm:text-2xl font-mono font-black text-emerald-950 mt-1">
      ...
      {/* Tabs navigation - Horizontal Scrollable with Clear Highlights */}
      <div className="flex items-center justify-between border-b-2 border-neutral-200 gap-2 overflow-x-auto pb-1 scrollbar-none">
        <div className="flex items-center gap-1.5 flex-nowrap min-w-max">
          ...
          >
            ...
            <span>1. Today's Daily Farm Details</span>
          </button>
          ...
          <button
            ...
            id="tab-farmer-transport"
          >
            <Truck className="w-4 h-4 text-violet-300" />
            <span>2. Nearby Transport Availability &amp; Booking</span>
          </button>
          ...
          >
            ...
            <span>3. Produce &amp; Inventory ({farmProducts.length})</span>
          </button>
          ...
          >
            ...
            <span>4. Orders &amp; Pickups ({farmOrders.length})</span>
          </button>
          ...
          >
            ...
            <span>5. Transparent ₹ Bank Ledger ({farmPayouts.length})</span>
          </button>
        </div>
        ...
        {activeTab === 'inventory' && (
          <button
            ...
            className="px-3.5 py-2 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl text-xs font-bold shadow-xs flex items-center gap-1.5 transition-colors whitespace-nowrap cursor-pointer"
      ...
      {/* TAB 0: Today's Daily Farm Details (Default First Screen) */}
      {activeTab === 'daily' && (
        <FarmerDailyOverview
          onNavigateTab={(tab) => setActiveTab(tab)}
          onOpenAddProduce={() => setIsAddModalOpen(true)}
          onOpenSchemes={() => setIsSchemesModalOpen(true)}
        />
      )}

      {/* TAB: Nearby Transport Availability & Vehicle Dispatch */}
      {activeTab === 'transport' && (
        <TransportAvailabilitySection onBookingSuccess={() => setActiveTab('daily')} />
      )}
          ...
          <div className="bg-white border-2 border-neutral-300 rounded-2xl overflow-hidden shadow-xs">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-neutral-100 text-neutral-700 font-bold border-b-2 border-neutral-300 uppercase text-[11px] tracking-wider">
                    <th className="py-3.5 px-4 bg-neutral-100 border-r border-neutral-200">1. Transaction / Date</th>
                    <th className="py-3.5 px-4 bg-neutral-100 border-r border-neutral-200">2. Order Ref</th>
                    <th className="py-3.5 px-4 bg-amber-50/80 border-r border-neutral-200 text-amber-950">3. Produce Sold</th>
                    <th className="py-3.5 px-4 bg-neutral-100 border-r border-neutral-200">4. Gross Sale (₹)</th>
                    <th className="py-3.5 px-4 bg-emerald-100/80 border-r border-neutral-200 text-emerald-950 font-black">5. Farmer Share (88% in ₹)</th>
                    <th className="py-3.5 px-4 bg-neutral-100 border-r border-neutral-200">6. Logistics (7%)</th>
                    <th className="py-3.5 px-4 bg-neutral-100 border-r border-neutral-200">7. Platform (5%)</th>
                    <th className="py-3.5 px-4 bg-emerald-50/80 text-right text-emerald-950">8. Settlement</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-neutral-150">
