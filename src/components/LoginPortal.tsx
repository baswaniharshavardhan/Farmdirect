    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 flex flex-col justify-center items-center p-4 sm:p-6 selection:bg-emerald-500 selection:text-white">
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="theme-heading font-serif-harvest text-3xl sm:text-4xl font-black tracking-tight text-white">
            FarmDirect
          </h1>
          <p className="text-xs sm:text-sm text-neutral-400 mt-0.5 font-sans">
            Regional Producer &amp; Cold-Chain Network
          </p>
          <div className="mt-1">
            <span className="font-script text-lg text-amber-300 font-bold drop-shadow-xs">
              ✨ 88% Direct Payout to Regional Kisans
            </span>
          </div>
        </div>
              ...
              <div>
                ...
                <div className="flex gap-1.5">
                  {demoAccounts.map((acc) => (
                    ...
                    >
                      {acc.name ? acc.name.split(' ')[0] : acc.label}
                    </button>
                  ))}
                </div>
              </div>

              {/* FARMER KISANSETU SHOWCASE: Added directly when Farmer role is selected */}
              {selectedRole === 'farmer' && (
                <div className="rounded-2xl overflow-hidden border-2 border-amber-500/60 bg-gradient-to-br from-[#033b2e] via-[#022c22] to-[#011c16] p-3.5 shadow-[0_12px_30px_rgba(245,158,11,0.25)] relative transition-all animate-in fade-in zoom-in-95 duration-200">
                  <div className="flex items-center gap-3.5">
                    <div className="relative w-16 h-16 rounded-xl overflow-hidden border-2 border-amber-400 flex-shrink-0 shadow-lg">
                      <img
                        src="/farmer-portrait.jpg"
                        alt="Rameshwar Lal Sharma - Kisan Mitra"
                        className="w-full h-full object-cover object-top"
                        referrerPolicy="no-referrer"
                      />
                      <div className="absolute bottom-0 inset-x-0 bg-amber-400 text-[9px] font-black text-center text-neutral-950 py-0.5 tracking-wider">
                        KISAN
                      </div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <h4 className="font-display font-black text-white text-sm truncate">
                          Rameshwar Lal Sharma
                        </h4>
                        <span className="px-1.5 py-0.2 rounded-full text-[9px] font-bold bg-amber-400 text-neutral-950">
                          SMART AGRI
                        </span>
                      </div>
                      <p className="text-[11px] text-amber-200/90 font-medium mt-0.5">
                        📍 Karnal, Haryana · 8.5 Acres Farmland
                      </p>
                      <div className="flex items-center gap-2 mt-1.5 text-[10px] text-emerald-300">
                        <span className="bg-emerald-950/80 px-2 py-0.5 rounded-md border border-emerald-600/40 font-bold">
                          🌾 Rabi 2026 ICAR Grounded
                        </span>
                        <span className="bg-emerald-950/80 px-2 py-0.5 rounded-md border border-emerald-600/40 font-bold">
                          ✨ AI Crop Doctor
                        </span>
