...
interface MarketplaceContextType {
  ...
  // Nearby Transport Availability & Vehicle Hire
  transporters: Transporter[];
  transportBookings: TransportBookingRequest[];
  bookTransport: (booking: Omit<TransportBookingRequest, 'id' | 'createdAt' | 'status'>) => TransportBookingRequest;
  registerTransporter: (transporter: Omit<Transporter, 'id' | 'rating' | 'totalTripsCompleted' | 'verifiedDriver' | 'avatar'>) => Transporter;
  toggleTransporterAvailability: (id: string) => void;
}
...
const STORAGE_KEY_TRANSPORTERS = 'farmdirect_transporters_v1';
const STORAGE_KEY_TRANSPORT_BOOKINGS = 'farmdirect_transport_bookings_v1';
...
export const MarketplaceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [transporters, setTransporters] = useState<Transporter[]>(() => {
    const saved = localStorage.getItem(STORAGE_KEY_TRANSPORTERS);
    return saved ? JSON.parse(saved) : INITIAL_TRANSPORTERS;
  });

  const [transportBookings, setTransportBookings] = useState<TransportBookingRequest[]>(() => {
    const saved = localStorage.getItem(STORAGE_KEY_TRANSPORT_BOOKINGS);
    return saved
      ? JSON.parse(saved)
      : [
          {
            ...
            transporterName: 'Ramu Shinde',
            vehicleType: 'Mini Truck (Tata Ace)',
            pickupLocation: 'Sunrise Organic Orchards, Nashik',
            destinationLocation: 'Nashik APMC Terminal Yard',
            cargoDetails: '15 Crates Fresh Apples & Lemons',
            weightQuintals: 4.5,
            pickupTime: 'Today 11:30 AM',
            estimatedKm: 14,
            estimatedFareRupees: 252,
            farmerName: 'Thomas Thorne',
            farmerPhone: '+91 94231 88910',
            status: 'Accepted',
  ...
  useEffect(() => {
    localStorage.setItem(STORAGE_KEY_TRANSPORTERS, JSON.stringify(transporters));
  }, [transporters]);

  useEffect(() => {
    localStorage.setItem(STORAGE_KEY_TRANSPORT_BOOKINGS, JSON.stringify(transportBookings));
  }, [transportBookings]);
  ...
  const registerCustomer = async (
    ...
    try {
      ...
      const newUser: User = {
        ...
        phone: data.phone?.trim() || '+91 98201 44552',
        ...
        buyerType: data.buyerType || 'normal',
        businessName: data.businessName?.trim(),
        gstNumber: data.gstNumber?.trim(),
        orderVolume: data.orderVolume?.trim(),
      };
  ...
  const registerFarmer = async (
    ...
    try {
      ...
      const newUser: User = {
        ...
        registeredAt: new Date().toISOString(),
        phone: data.phone?.trim() || '+91 94231 88910',
        fpoCode: data.fpoCode?.trim() || 'FPO-MH-2024-7890',
        fpoName: data.fpoName?.trim() || 'Sahyadri Farmers Producer Co.',
        fpoCluster: data.fpoCluster?.trim() || 'Nashik Horticulture Cluster',
      };
  ...
  const bookTransport = (booking: Omit<TransportBookingRequest, 'id' | 'createdAt' | 'status'>): TransportBookingRequest => {
    const newBooking: TransportBookingRequest = {
      ...booking,
      id: `bk-${Date.now()}`,
      createdAt: new Date().toISOString(),
      status: 'Accepted',
    ...
    setSystemNotification(`🚚 Transport booked! Driver ${newBooking.transporterName} confirmed.`);
    return newBooking;
  };

  const registerTransporter = (
    tData: Omit<Transporter, 'id' | 'rating' | 'totalTripsCompleted' | 'verifiedDriver' | 'avatar'>
  ): Transporter => {
    ...
    setSystemNotification(`🚚 Vehicle ${newTransporter.vehicleNumber} registered successfully!`);
    return newTransporter;
  };

  const toggleTransporterAvailability = (id: string) => {
    setTransporters((prev) =>
      prev.map((t) => (t.id === id ? { ...t, availableNow: !t.availableNow } : t))
    );
  };
  ...
  const resetDatabase = () => {
    ...
    localStorage.removeItem(STORAGE_KEY_TRANSPORTERS);
    localStorage.removeItem(STORAGE_KEY_TRANSPORT_BOOKINGS);
