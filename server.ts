...
app.post(['/api/signup', '/api/auth/signup'], (req, res) => {
  ...
  const newUser: UserRecord = {
    ...
    buyerType: buyerType || (cleanRole === 'customer' ? 'normal' : undefined),
    businessName: businessName?.trim(),
    gstNumber: gstNumber?.trim(),
    orderVolume: orderVolume?.trim(),
    fpoCode: fpoCode?.trim(),
    fpoName: fpoName?.trim(),
    fpoCluster: fpoCluster?.trim(),
  };
  ...
  res.status(201).json({
    ...
    user: {
      ...
      orderVolume: newUser.orderVolume,
      fpoCode: newUser.fpoCode,
      fpoName: newUser.fpoName,
      fpoCluster: newUser.fpoCluster,
    },
    role: newUser.role,
    redirectUrl,
  });
...
app.post('/api/auth/send-otp', (req, res) => {
  ...
  console.log(`[OTP] Generated OTP ${code} for phone ${cleanPhone}`);

  res.json({
    success: true,
    message: `Verification code sent to ${phone}`,
    code, // Demo display code
...
app.post('/api/auth/verify-otp', (req, res) => {
  ...
  const isValid =
    cleanOtp === '123456' ||
    (record && record.code === cleanOtp && record.expiresAt > Date.now());
  ...
  return res.status(400).json({
    success: false,
    verified: false,
    error: 'Invalid or expired OTP code. Use the code shown or 123456.',
  });
});
...
// Phase 7: Gemini AI Maps Grounding (using gemini-3.5-flash with googleMaps tool)
app.post('/api/ai/maps-grounding', async (req, res) => {
  ...
  try {
        ...
        mapsPlaces.push({
          ...
          reviewSnippets:
            chunk.maps.placeAnswerSources?.reviewSnippets?.map(
              (r: any) => (typeof r === 'string' ? r : r.snippet || '')
            ) || [],
        });
      }
    }
    ...
    res.status(500).json({
      ...
      requiresKey: isKeyError,
    });
  }
});

// Phase 7: Gemini AI Search Grounding (using gemini-3.5-flash with googleSearch tool)
app.post('/api/ai/search-grounding', async (req, res) => {
  ...
  try {
    ...
    // Extract web sources & search queries
